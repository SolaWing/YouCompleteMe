...
class CompletionRequest( BaseRequest ):
  def __init__( self, request_data ):
    ...
    self._complete_done_item = None # complete_done item cache
  ...
  def OnCompleteDone( self, reallyDone = False ):
    if not self.Done():
      return
  ...
  def CompletedItem( self ):
    # cache slow vim evaluate
    if self._complete_done_item is not None:
       return self._complete_done_item
    self._complete_done_item = vimsupport.GetVariableValue( 'v:completed_item' )
    return self._complete_done_item

  def _GetExtraDataUserMayHaveCompleted( self ):
    completed_item = self.CompletedItem()
...
def _GetCompletionMenuField( completion_data ):
  if 'menu_text' in completion_data:
      abbr = completion_data[ 'menu_text' ]
  else:
      # vim completed_item always set abbr, so provide one to ensure completionDone item is same with generated field
      abbr = completion_data[ 'insertion_text' ]
  # limit abbr field. (so extra info can be see and won't too long)
...
def _ConvertCompletionDataToVimData( completion_data ):
  ...
  return {
    ...
    'abbr'     : _GetCompletionMenuField( completion_data ),
